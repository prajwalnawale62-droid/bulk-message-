    ...
    try {
      ...
      if (isSignUp) {
        ...
        if (data.user) {
          ...
          try {
            // Log to database - Wrapped in try-catch as table might not exist
            try {
              ...
              console.warn("Could not log login to database (table might be missing)", dbErr);
            }
...
function SettingsView({ user, profile, onUpdate, onOpenModal, showNotify }: { user: any, profile: any, onUpdate: () => void, onOpenModal: (type: any) => void, showNotify: (m: string, t?: 'success' | 'error' | 'info' | 'warning') => void }) {
  ...
  const checkStatus = async (url: string) => {
    try {
      const response = await axios.get(`/api/whatsapp-server/status?email=${encodeURIComponent(user?.email || 'default')}`);
      if (response.data.connected === true) {
        ...
        const qrResponse = await axios.get(`/api/whatsapp-server/qr?email=${encodeURIComponent(user?.email || 'default')}`);
  ...
  const fetchQRCode = async () => {
    try {
      const res = await fetch(`/api/whatsapp-server/qr?email=${encodeURIComponent(user?.email || 'default')}`);
...
const DashboardView = ({ user, profile, setView }: { user: any, profile: any, setView: (v: View) => void }) => {
  ...
  const fetchQR = async () => {
    try {
      const response = await axios.get(`/api/whatsapp-server/qr?email=${encodeURIComponent(user?.email || 'default')}`);
      if (response.data.qr) {
